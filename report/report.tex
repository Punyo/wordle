...
\subsection{FXMLファイル}
このセクションでは、JavaFXを使用して作成されたFXMLファイルについて説明する。それぞれのFXMLファイルは、対応するコントローラクラスと連携して画面のレイアウトや動作を定義している。

\subsubsection{game.fxml}
プレイ画面のレイアウトを定義するFXMLファイル。このファイルでは以下の要素が含まれる。
\begin{itemize}
  \item 単語入力用のテキストフィールド
  \item 入力結果を表示するグリッド
  \item 残り試行回数を表示するラベル
  \item 「送信」ボタン
\end{itemize}
...
メニュー画面のレイアウトを定義するFXMLファイル。このファイルでは以下の要素が含まれる。
\begin{itemize}
  \item 「新規プレイ」ボタン
  \item 「履歴確認」ボタン
  \item 「終了」ボタン
...
記録画面のレイアウトを定義するFXMLファイル。このファイルでは以下の要素が含まれる。
\begin{itemize}
  \item プレイ履歴を表示するテーブルビュー
  \item 各列には以下の情報を表示
  \begin{itemize}
    \item プレイ日時
    \item 正解単語
...
ゲーム終了時に表示されるダイアログ画面のレイアウトを定義するFXMLファイル。このファイルでは以下の要素が含まれる。
\begin{itemize}
  \item ゲーム結果（成功/失敗）を表示するラベル
  \item 正解単語を表示するラベル
  \item 「メニューに戻る」ボタン
  \item 「再プレイ」ボタン
\end{itemize}
